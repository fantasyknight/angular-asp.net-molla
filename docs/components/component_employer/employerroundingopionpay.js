...
const _class = require("../../model/clsemployerroundingopionpay");
...
return { 'flag': true, 'query': strquery };

} catch (error) {
return { 'flag': false, 'query': error };
}
},
...
let strquery = _class.data.select(" and employerroundingopionpayId = " + verb.employerroundingopionpayId);
return { 'flag': true, 'query': strquery };
...
return { 'flag': true, 'query':  strquery + ";" + strcount };
...
let verb = _class.data.masterData(request);
let strquery = _class.data.delete(verb.employerroundingopionpayId);

let strCount = _class.data.getcount("");
let [strCountResult, _strCountResult] = await dbSecurity.asyncResult(strCount);

return { 'flag': true, 'count': strCountResult, 'query': strquery };
